...
import com.github.drmarjanovic.contacts.api.Contacts
import com.github.drmarjanovic.contacts.config.Config.DatabaseConfig
import com.github.drmarjanovic.contacts.config.Configuration
import com.github.drmarjanovic.contacts.domain.ContactService
import com.github.drmarjanovic.contacts.infrastructure.PgContactRepo
...
object Server extends ZApp {
  ...
  override def run(args: List[String]): ZIO[ZEnv, Nothing, Int] =
    (for {
      ...
      routes  = Router[AppTask](Api -> new Contacts(service).routes()).orNotFound
      ...
      zio <- tracerR.use { jaeger =>
              transactorR.use { transactor =>
                server.provideSome[ZEnv] { base =>
                  new Blocking with Clock with PgContactRepo with OpenTracing {
